...
/// How the chord HUD is triggered.
///
/// - `longPressSpace`: original behaviour. Hold Space ~200ms to enter chord mode.
///   Works without any modifier but Space is swallowed in password fields by
///   macOS SecureInput, so the trigger silently fails there.
/// - `controlSpace` / `optionSpace`: tap ⌃Space or ⌥Space (leader-style). Plain
///   Space passes through untouched, so password fields are unaffected. Chord
///   mode stays open until a bound key fires, ESC cancels, an unbound key is
///   pressed, or `leaderTimeout` elapses.
enum SpaceShortcutTriggerMode: String, Codable, CaseIterable {
    case longPressSpace
    case controlSpace
    case optionSpace
}
...
/// Transient event tap that captures a single chord key after the leader
/// hotkey (⌃Space / ⌥Space) fires. Installed only while the HUD is open, so
/// the rest of the time the keyboard stream isn't observed at all and plain
/// Space is never even seen by us. Calls `onEnd` exactly once when chord mode
/// ends for any reason (key fired, ESC, unbound key, timeout, or explicit end).
@MainActor
final class LeaderChordCapture {

    typealias Handler = (UInt16) -> Bool   // returns true if a binding fired
    ...
    /// Key whose keyDown we just swallowed (chord fired). We also swallow its
    /// keyUp, then tear down.
    private var pendingFireKey: UInt16?
    ...
    func begin() -> Bool {
        ...
        guard let tap = CGEvent.tapCreate(
            ...
            options: .defaultTap,
            eventsOfInterest: mask,
            callback: LeaderChordCapture.callback,
            userInfo: userInfo
        ) else {
            return false
        }
    ...
    private static let callback: CGEventTapCallBack = { _, type, event, userInfo in
        guard let userInfo = userInfo else { return Unmanaged.passUnretained(event) }
        let capture = Unmanaged<LeaderChordCapture>.fromOpaque(userInfo).takeUnretainedValue()
        return MainActor.assumeIsolated {
    ...
    private func handle(type: CGEventType, event: CGEvent) -> Unmanaged<CGEvent>? {
        ...
        // Mop up the keyUp (and any autorepeat keyDown) for the chord key we
        // just fired, then tear down.
        if let pending = pendingFireKey, pending == keyCode {
            if type == .keyUp {
                end()
            }
            return nil
        ...
        // Bound key → fire, swallow keyDown, wait for matching keyUp.
        if let handler, handler(keyCode) {
            pendingFireKey = keyCode
            cancelTimeout()
            return nil
        }

        // Unbound key → let it through and end.
        end()
        return Unmanaged.passUnretained(event)
    }
}
